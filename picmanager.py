...
import time as t_
import wx
import wx.animate as wx_animate
...
def i_round(x, d=0): return int(round(x, d))


def path_to_name(path):
    if os.path.isfile(path):
        return path.rsplit(os.sep)[-1]
    return ''


def show_tip(parent, message, title=u'提示', style=wx.OK | wx.CENTRE):
    tip = wx.MessageDialog(parent, message, title, style)
    tip.ShowModal()
    tip.Destroy()


def frame_fadeout(frame):
    ...
    #     if i != 255:
    #         print t_.clock()-startT,'40'
    #     t_.sleep(0.001)
    #     print t_.clock()-startT, '42'
...
class WaitingFrame(wx.MiniFrame):
    ...
    def __init__(self, control):
        super(self.__class__, self).__init__(app.mFrame, size=mainC.waitingFrameSize,
                                             style=wx.NO_BORDER | wx.STAY_ON_TOP)
        self.Center()
        # self.SetTransparent(150)
        self.SetBackgroundColour('white')
        ani = wx_animate.Animation(mainC.waitingIm)
        ctrl = wx_animate.AnimationCtrl(self, -1, ani)
...
class MyMenu(object):
    ...
    def __init__(self, parent, menu_data, _type, font_size=9):
        super(self.__class__, self).__init__()
        self.parent = parent
        if _type == 'menubar':
            self.__obj = self.createMenuBar(menu_data)
        elif _type == 'popupMenu':
            self.__obj = self.createMenu(menu_data)
        else:
            self.__obj = wx.MenuBar()
        if hasattr(self.__obj, 'SetFont'):
            pass
            # self.__obj.SetFont(wx.Font(font_size, wx.FONTFAMILY_MAX, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL))
...
class ImContainer(wx.Window):
    ...
    def __init__(self, parent, label='', path=mainC.welIm, selected=False, id=-1, pos=wx.DefaultPosition,
                 size=mainC.imConSize):
    ...
    def initText(self):  # 已经被替代
        ...
        if hasattr(self, 'nameText'):
            ...
            nt = self.nameText = wx.StaticText(self, -1, path_to_name(self.imPath), (i_round(sz.x * 0.15), sz.y - 45),
                                               (sz.x, 30), wx.ALIGN_CENTER)
            nt.Wrap(i_round(sz.x * 0.7))

    def init_bitmap(self, image_changed=0):
        sz = self.GetSize()
        self.impos = [4, 4]
        ...
        #         self.Refresh(0)
        #         del self.pic
        #     gif = wx_animate.Animation(self.imPath)
        #     self.gifC = wx_animate.AnimationCtrl(self, -1, gif, (self.impos[0], self.impos[1]))
        #     self.gifC.Play()
        #     #wx.Yield()
        #     return
    ...
    def initBuffer(self, brush='sel'):
        ...
        dc.Clear()
        if self.selected or brush == 'hover' and self.HitTest(
                        wx.GetMousePosition() - self.GetScreenPosition()) == wx.HT_WINDOW_INSIDE:
            dc.SetPen(self.pen)
        ...
        name = path_to_name(self.imPath)
        tn = i_round((sz.x - 100) / 6) + 1
        name = name if len(name) < tn else name[:tn - 1] + '...'
        # namepos=[Iround(sz.x*0.15)]
        dc.DrawText(name, i_round((sz.x - 6 * len(name)) / 2), sz.y - 26)
        self.reInitBuffer = 0
...
class MainWindow(wx.SplitterWindow):
    ...
    def formFolderList(self, path=''):
        ...
        for x in tr:
            ...
            if not data:
                ...
                pos = [path] + x[0][len(path) + 1:].split(sep)
                posStr = 'data[1]'
                indexs = []
    ...
    def ItemGetPath(self, item):
        ...
        while fl.GetItemParent(curItem) != fl.GetRootItem():
            ...
            path = fl.GetItemText(curItem) + sep + path
        # print path,' 2'
        return path
...
class MainFrame(wx.Frame):
    ...
    def setFolders(self, folders):
        ...
        if mainC.listFolder != old:
            ...
            show_tip(self, u'选择的文件重复或已包含，若要缩小文件夹范围请先删除已包含的父文件夹!', u'提示')
    ...
    def OnOpen(self, e):
        ...
        if openD.ShowModal() == wx.ID_OK:
            ...
            paths = filter(lambda item: item.find(sep) > -1, paths)
            if paths:
                self.setFolders(paths)
            else:
                show_tip(self, u'文件夹暂不支持整个硬盘分区直接加入')
        openD.Destroy()
    ...
    def OnClose(self, e):
        self.window.folderList.SelectItem(
            self.window.folderList.GetLastChild(self.window.folderList.GetRootItem()))  # 消除关闭时，文件列表错误
